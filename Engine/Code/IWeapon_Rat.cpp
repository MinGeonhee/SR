...
void IWeapon_Rat::Initialize(CWeaponController* pWeapon)	// 
{
	if (!pWeapon)
		return;
...
void IWeapon_Rat::Enter(CWeaponController* pWeapon)
{
	m_fElapsedTime = 0.f;
	m_fFireRate = 3.f; // 무기 발사 속도 3초
	m_iMax_BulletCount = 100;	// 최대 불릿 카운트 100개
...
void IWeapon_Rat::Update(CWeaponController* pWeapon, float fDeltaTime)
{
	__super::IWeapon_Update(fDeltaTime);	// 기본적으로 돌아가야하는 로직들.
}


void IWeapon_Rat::Exit(CWeaponController* pWeapon)
{

}

void IWeapon_Rat::Fire(CWeaponController* pWeapon)
{
		...
		bullet->Change_BulletState(BULLET_RAT);
