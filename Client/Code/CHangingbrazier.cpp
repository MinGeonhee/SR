...
CHangingbrazier::CHangingbrazier(LPDIRECT3DDEVICE9 Device)
...
CHangingbrazier::CHangingbrazier(const CHangingbrazier& cpy)
    : CLightObject(cpy)
{
    m_tHandle.m_eType = OBJTYPE::OBJ_LIGHT;
...
CHangingbrazier::~CHangingbrazier()
{
}

HRESULT CHangingbrazier::Ready_GameObject()
{
    if (FAILED(CHangingbrazier::Add_Component()))
        return E_FAIL;


    m_pTextureCom = CResourceManager::GetInstance()->Get<CTexture>(L"HANGINGBRAZIER");


    m_pAnimationCom->SetGrid(8,4); // 8열 4행
...
_int CHangingbrazier::Update_GameObject(const _float& fTimeDelta)
{
...
void CHangingbrazier::LateUpdate_GameObject(const _float& fTimeDelta)
{
    m_pBillboardCom->LateUpdate_Component(fTimeDelta);

}

void CHangingbrazier::Render_GameObject()
{
...
CHangingbrazier* CHangingbrazier::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
    CHangingbrazier* pObj = new CHangingbrazier(pGraphicDev);

    if (FAILED(pObj->Ready_GameObject()))
    {
        Safe_Release(pObj);
        MSG_BOX("CHangingbrazier Create Failed");
        return nullptr;
    }
...
HRESULT CHangingbrazier::Add_Component()
{
