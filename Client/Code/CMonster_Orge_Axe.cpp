...
CMonster_Orge_Axe::CMonster_Orge_Axe(const CMonster_Orge_Axe& rhs)
	:CMonster(rhs)
{
}

CMonster_Orge_Axe::CMonster_Orge_Axe(LPDIRECT3DDEVICE9 pGraphicDev)
	:CMonster(pGraphicDev)
{
}

CMonster_Orge_Axe::~CMonster_Orge_Axe()
{
}

HRESULT CMonster_Orge_Axe::Ready_GameObject()
{
	...
	m_tHandle.m_eType = OBJ_MONSTER_ORGE_AXE;
	Set_MonsterType(MONSTER_ORGE_AXE);
	Set_MonsterRank(MONSTERRANK::RANK_COMMON);

	m_fSpeed = 2.f;
	m_fPlayerRecog_Distance = 10.f;
	m_fAttack_Distance = 0.8f;
	...
	m_pBuff->Set_Offset_Y(0.8f);

	return S_OK;
}

_int CMonster_Orge_Axe::Update_GameObject(const _float& fTimeDelta)
{
	...
	if (m_eMonsterStatus == MONSTER_STATUS_ATTACK ||
		m_eMonsterStatus == MONSTER_STATUS_HIT)
	{
...
void CMonster_Orge_Axe::LateUpdate_GameObject(const _float& fTimeDelta)
{
...
void CMonster_Orge_Axe::Render_GameObject()
{
	m_pGraphicDev->SetTransform(D3DTS_WORLD, m_pTransform->Get_World());
	CResourceManager::GetInstance()->Get<CTexture>(L"Orge_Axe_Animation")->Set_Texture();
...
void CMonster_Orge_Axe::Chase_Player(const _float& fTimeDelta)
{
...
void CMonster_Orge_Axe::Set_Animation()
{
	// 상태 안바뀌었으면 걍 리턴
	...
	case(MONSTER_STATUS::MONSTER_STATUS_MOVE):
	{
		...
		{
			m_pAnimationCom->AniMation_Init(6, 0.15f);
			m_fSpeed = 2.f;
		}
		m_pAnimationCom->SetPlayMode(CAnimation::PlayMode::Loop);
		m_pTransform->Set_Scale(_vec3{ 1.f, 1.f, 1.f });
	...
	case(MONSTER_STATUS::MONSTER_STATUS_ATTACK):
	{
		...
		{
			m_pAnimationCom->AniMation_Init(8, 0.11f);
		}
		m_pAnimationCom->SetPlayMode(CAnimation::PlayMode::Once);
		m_pTransform->Set_Scale(_vec3{ 1.f, 1.f, 1.f });
		m_pAnimationCom->PlayAnimation();
...
HRESULT CMonster_Orge_Axe::Add_Component()
{
	...
	m_pWeaponCom->Change_Weapon(WEAPON_ORGE_AXE);
...
CMonster_Orge_Axe* CMonster_Orge_Axe::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CMonster_Orge_Axe* pMonster = new CMonster_Orge_Axe(pGraphicDev);
...
void CMonster_Orge_Axe::Play_BloodParticleSystem()
{
...
void CMonster_Orge_Axe::Play_IceParticleSystem()
{
