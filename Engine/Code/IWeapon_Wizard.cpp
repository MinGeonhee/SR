...
void IWeapon_Wizard::Initialize(CWeaponController* pWeapon)	// 
{
	if (!pWeapon)
		return;
...
void IWeapon_Wizard::Enter(CWeaponController* pWeapon)
{
	m_fElapsedTime = 0.f;
	m_fFireRate = 3.f; // 무기 발사 속도 3초
	m_iMax_BulletCount = 100;	// 최대 불릿 카운트 100개
...
void IWeapon_Wizard::Update(CWeaponController* pWeapon, float fDeltaTime)
{
	__super::IWeapon_Update(fDeltaTime);	// 기본적으로 돌아가야하는 로직들.
}


void IWeapon_Wizard::Exit(CWeaponController* pWeapon)
{

}

void IWeapon_Wizard::Fire(CWeaponController* pWeapon)
{
		...
		_vec3 BulletDirOffest = { 0.f,-0.11f,0.f };

		auto bullet = static_cast<CBullet*>(CObjectPoolingManager::GetInstance()->Get(OBJ_BULLET));

		if (!bullet) return;

		pos = pos + _vec3{ 0.f,0.3f,0.f };
		bullet->Change_BulletState(BULLET_WIZARD);
