...
CMonster_Goblin_Hammer::CMonster_Goblin_Hammer(const CMonster_Goblin_Hammer& rhs)
	:CMonster(rhs)
{
}

CMonster_Goblin_Hammer::CMonster_Goblin_Hammer(LPDIRECT3DDEVICE9 pGraphicDev)
	:CMonster(pGraphicDev)
{
}

CMonster_Goblin_Hammer::~CMonster_Goblin_Hammer()
{
}

HRESULT CMonster_Goblin_Hammer::Ready_GameObject()
{
	...
	m_tHandle.m_eType = OBJ_MONSTER_GOBLIN_HAMMER;
	Set_MonsterType(MONSTER_GOBLIN_HAMMER);
...
_int CMonster_Goblin_Hammer::Update_GameObject(const _float& fTimeDelta)
{
...
void CMonster_Goblin_Hammer::LateUpdate_GameObject(const _float& fTimeDelta)
{
...
void CMonster_Goblin_Hammer::Render_GameObject()
{
	m_pGraphicDev->SetTransform(D3DTS_WORLD, m_pTransform->Get_World());
	CResourceManager::GetInstance()->Get<CTexture>(L"Goblin_Hammer_Animation")->Set_Texture();
...
void CMonster_Goblin_Hammer::Chase_Player(const _float& fTimeDelta)
{
...
void CMonster_Goblin_Hammer::Set_Animation()
{
	// 상태 안바뀌었으면 걍 리턴
	...
	case(MONSTER_STATUS::MONSTER_STATUS_MOVE):
	{
		...
		if (m_bIsBuffed)
		{
			m_pAnimationCom->AniMation_Init(6, 0.2f);	// 몇 열이고 출력 속도 몇인지
			m_fSpeed = 1.5f;
		}
		else
		{
			m_pAnimationCom->AniMation_Init(6, 0.25f);
			m_fSpeed = 1.f;
		}
		m_pAnimationCom->SetPlayMode(CAnimation::PlayMode::Once);
		m_pTransform->Set_Scale(_vec3{ 1.f, 1.f, 1.f });
	...
	case(MONSTER_STATUS::MONSTER_STATUS_ATTACK):
	{
		...
		if (m_bIsBuffed)
		{
			m_pAnimationCom->AniMation_Init(8, 0.07f);	// 몇 열이고 출력 속도 몇인지
		}
		else
		{
			m_pAnimationCom->AniMation_Init(8, 0.12f);
		}
		m_pAnimationCom->SetPlayMode(CAnimation::PlayMode::Once);
		m_pTransform->Set_Scale(_vec3{ 1.f, 1.f, 1.f });
		m_pAnimationCom->PlayAnimation();
...
void CMonster_Goblin_Hammer::Attack()
{
...
HRESULT CMonster_Goblin_Hammer::Add_Component()
{
	...
	m_pWeaponCom->Change_Weapon(WEAPON_GOBLIN_HAMMER);
...
CMonster_Goblin_Hammer* CMonster_Goblin_Hammer::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CMonster_Goblin_Hammer* pMonster = new CMonster_Goblin_Hammer(pGraphicDev);
...
void CMonster_Goblin_Hammer::Free()
{
...
void CMonster_Goblin_Hammer::Play_BloodParticleSystem()
{
...
void CMonster_Goblin_Hammer::Play_IceParticleSystem()
{
