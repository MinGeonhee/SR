...
CStatue::CStatue(LPDIRECT3DDEVICE9 Device)
...
CStatue::CStatue(const CStatue& cpy)
    : CLightObject(cpy) 
{
    m_tHandle.m_eType = OBJTYPE::OBJ_LIGHT;
    m_eLight_Object_Type = LIGHT_OBJECT_TYPE::STATUE;
...
HRESULT CStatue::Ready_GameObject()
{
    if (FAILED(CStatue::Add_Component()))
        return E_FAIL;


    m_pTextureCom = CResourceManager::GetInstance()->Get<CTexture>(L"STATUE");
    ...
    m_pAnimationCom->SetCol(0,3);      // 맨 위 행
...
void CStatue::LateUpdate_GameObject(const _float& fTimeDelta)
{
...
CStatue* CStatue::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
    CStatue* pObj = new CStatue(pGraphicDev);

    if ( FAILED(pObj->Ready_GameObject()))
    {
        Safe_Release(pObj);
        MSG_BOX("CStatue Create Failed");
        return nullptr;
    }
