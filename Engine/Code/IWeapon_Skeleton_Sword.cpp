...
void IWeapon_Skeleton_Sword::Initialize(CWeaponController* pWeapon)	// 
{
	if (!pWeapon)
		return;
...
void IWeapon_Skeleton_Sword::Enter(CWeaponController* pWeapon)
{
	m_fElapsedTime = 0.f;
	m_fFireRate = 3.f; // 무기 발사 속도 3초
	m_iMax_BulletCount = 100;	// 최대 불릿 카운트 100개
...
void IWeapon_Skeleton_Sword::Update(CWeaponController* pWeapon, float fDeltaTime)
{
	__super::IWeapon_Update(fDeltaTime);	// 기본적으로 돌아가야하는 로직들.
}


void IWeapon_Skeleton_Sword::Exit(CWeaponController* pWeapon)
{

}

void IWeapon_Skeleton_Sword::Fire(CWeaponController* pWeapon)
{
		...
		bullet->Change_BulletState(BULLET_SKELETON_SWORD);
