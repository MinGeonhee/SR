...
void IWeapon_Boss_IceBall::Initialize(CWeaponController* pWeapon)	// 
{
	if (!pWeapon)
		return;
...
void IWeapon_Boss_IceBall::Enter(CWeaponController* pWeapon)
{
	m_fElapsedTime = 0.f;
	m_fFireRate = 3.f; // 무기 발사 속도 3초
	m_iMax_BulletCount = 100;	// 최대 불릿 카운트 100개
...
void IWeapon_Boss_IceBall::Update(CWeaponController* pWeapon, float fDeltaTime)
{
	__super::IWeapon_Update(fDeltaTime);	// 기본적으로 돌아가야하는 로직들.
}


void IWeapon_Boss_IceBall::Exit(CWeaponController* pWeapon)
{

}

void IWeapon_Boss_IceBall::Fire(CWeaponController* pWeapon)
{
	//if (m_bCanFire)	// 쏠 수 있는 상태입니다
	{
		...
		bullet->Change_BulletState(BULLET_BOSS_ICEBALL);
...
void IWeapon_Boss_IceBall::Overdrive_Fire(CWeaponController* pWeapon)
{
	if (m_bCanOverDrive)	// 쏠 수 있는 상태입니다
	{
		auto Owner = pWeapon->Get_MyOwner();
		auto pos = *(m_vMuzzleList[0].tTranfrom->GetInfoPos()) + m_vMuzzleList[0].tOffset;
